...
export default function Tabs({ children, currentRole, ...props }) {
  ...
  return (
    <div>
      <div className={twMerge("mb-4 ml-1 flex items-center", isMenuExpanded ? "justify-between" : "justify-center")}>
        <ImgWithWrapper
          imageAttributes={{ src: "/assets/satyata_text_logo.png", alt: "logo", hidden: !isMenuExpanded }}
          imageClassName="object-contain object-left"
          wrapperClassName={twMerge(
            "h-[60px] transition-[width] duration-300",
            isMenuExpanded ? "block w-[calc(1.4*60px)]" : "w-0", // aspect-[1.4]
          )}
        />
...
const TabItem = ({ className, children, accessableBy, ...props }) => {
  ...
  return (
    <li
      className={twMerge(
        "group whitespace-nowrap rounded-md px-2 py-2 font-semibold transition-all duration-300 hover:bg-[#01424c]",
        className,
      )}
      {...props}
    >
