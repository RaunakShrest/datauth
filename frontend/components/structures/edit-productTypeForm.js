    ...
    async function fetchProductTypeDetails() {
      ...
      try {
        const response = await axios.get(
          `${process.env.NEXT_PUBLIC_BASE_URL_DEV}/product-types/getProductTypeById/${productTypeId}`,
        )
    ...
    for (const key in formData) {
      if (key === "attributes") {
        const updatedAttributes = formData.attributes.map(
          (attr, index) => attr.attributeName !== productType.attributes?.[index]?.attributeName,
        )
          ...
          {formData.attributes.map((attribute, index) => (
            ...
            >
              <input
                ...
                className="my-2 w-full rounded-sm border border-black px-2 py-1"
              />
            </div>
          ))}
        </div>
