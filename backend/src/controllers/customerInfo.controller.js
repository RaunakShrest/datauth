  ...
  try {
    ...
    if (existingCustomer) {
      ...
      if (isDataSame) {
        ...
        return res.status(409).json(new ApiResponse(409, null, "Customer data already exists. No changes made."));
      }
      ...
      return res.status(200).json(new ApiResponse(200, existingCustomer, "Customer info updated successfully"));
    } else {
      ...
      return res.status(201).json(new ApiResponse(201, newCustomerInfo, "Customer info created successfully"));
    }
  } catch (error) {
    next(error);
  }
...
const getCustomerInfo = async (req, res, next) => {
  try {
    ...
    const customerInfo = await CustomerInfoModel.findOne({ soldProducts: productId });
    if (!customerInfo) {
      throw new ApiError(404, "Customer info not found for this product");
    }

    // Return the customer info
    res.status(200).json(new ApiResponse(200, "Customer info retrieved successfully", customerInfo));
  } catch (error) {
    next(error);
  }
};
...
export { postCustomerInfo, getSoldProductsByRetailer,getCustomerInfo };
